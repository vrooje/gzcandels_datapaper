...
\caption{
Depth corrections to classifications for individual responses to task T00: ``smooth'' (top row), ``features or disk'' (middle row), and ``star or artifact'' (bottom row). The change in classification, $\Delta f$, between the deep- and wide-field-depth observations of the same subjects, is fit as a function of deep-field-depth morphology (vote fraction $f$; left column) and galaxy characteristic surface brightness \musb . Each point is a subject which received at least 10 responses within the task. The best-fit plane is shown in projection (blue line) in the right-hand column. Fits were performed only for tasks where at least 75 subjects had 10 or more responses to the task; otherwise the correction is assumed to be 0. 
}
\label{fig:depthcorr}
\end{figure*}
%%%%% END FIGURE %%%%%
