...
In addition to the vote fractions for each subject, we provide a set of flags for each subject that indicates its member or non-member status in a ``clean'' sample of galaxies of a specific type. We select separate clean samples of smooth, featured, clumpy, edge-on, and spiral galaxies. These samples contain exemplars of each galaxy type with minimal contamination of the sample --- as a result, samples selected with the flags will be highly incomplete, but also highly pure. They are selected according to vote fraction and vote count thresholds given in Table \ref{table:clean} \notebsm{, which were selected following visual inspection of the resulting samples. The thresholds roughly coincide with advice given in \citet{kartaltepe15}, who consider classifications for systems with $\mathrm{H}\gt 24.4$ unreliable; only 8 systems in the ``clean'' spiral sample (2\% of the sample) are fainter than that limit.}.

We provide these flags for the convenience of the end user, but we additionally encourage those wishing to use Galaxy Zoo classifications to investigate whether a different set of thresholds would be optimal for their own science case. \notebsm{It should be noted that the redshift and magnitude distributions of samples constructed using these thresholds will be different from the main sample. We show in INSERT FIGURE HERE the distribution of galaxies identified as featured with a range of different thresholds; brighter, nearby galaxies are clearly more likely to appear in each sample.  \citep{bamford09} were able to adjust for this bias in classifications of SDSS galaxies by assuming that no evolution took place across the sample, something whi	ch is clearly not the case here. The study of Galaxy Zoo classifications in other Hubble surveys by Willett et al. (2016, in prep) used artifically redshifted images to quantify this bias, but no such images are yet available for the range of redshifts probed by CANDELS. The distribution of both redshift and brightness are relatively insensitive to choice of threshold (above 0.5), suggesting that galaxies classified as featured with any reasonable threshold represent robust detections. 

For smooth galaxies, as expected, the effect on redshift and brightness distributions is much smaller, with galaxies across the complete range of both variables included in the ``clean'' sample. }
...
%%%%%%%%% [FIGURE: dM2] %%%%%%%%%
\begin{figure*}
\includegraphics[scale=0.85]{dm_histogram_weighted_x10a_Simard.png}
\caption{
Weighted distributions of $M - M^*$ for the samples shown in figure \ref{fig:dM}, with weights chosen so that all samples have statistically indistinguishable $M-M^*$ distributions. Weights are normalised separately for smooth and featured distributions and chosen so that the maximum weight for any single galaxy is 1.0; this results in effective counts of 135 galaxies per smooth sample and 45 galaxies per featured sample (CANDELS and SDSS are shown as solid and dashed curves, respectively). The peak of the distribution is slightly fainter than $M^*$ at each redshift, and the full weighted distribution includes galaxies at luminosities $\pm 2$ magnitudes from $M^*$.
}
...
In order to meaningfully compare disk fractions between samples of different morphologies and redshifts, we require that \notebds{both featured and smooth} samples be matched in terms of how they sample the luminosity function of galaxies within their redshift ranges. Figures \ref{fig:dM}a and \ref{fig:dM}b show that the samples all have very different distributions of $\Delta M$. \notebds{We therefore weight galaxies in each samples such that the weighted histograms of $\Delta M$ are statistically indstinguishable, as shown in figure \ref{fig:dm2}. This method has the advantage of considering all the galaxies in the sample (with appropriate weights) rather than relying on randomly selected sub-samples.}
...
\notebsm{The existence of this population makes the common task of selecting a sample to study the properties of disk galaxies more difficult. Disk galaxies with smooth light distributions will be most cleanly selected by using a cut on the bulge-to-total ratio. However, those with complex features in the disk, whether these are spiral arms at low-redshift or clumpy galaxies in higher-redshift samples, may suffer from catastrophic faliure of fitting if modelled with smooth distributions. Therefore, a superset of galaxies selected from both Galaxy Zoo morphologies and smooth disks from cuts on bulge-to-total ratios will provide a more complete sample of $z > 1$ disks than either method alone. The subset of 1,096 ``smooth'' galaxies at $1 \leq z \leq 3$ with $IJH$ B/Tot$<0.5$ is shown in Table \ref{table:smooth_disks} and is available as part of this Galaxy Zoo CANDELS data release. 
