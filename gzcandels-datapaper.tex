...
Subsequent analysis has produced more refined photometry in each field (GOODS-S, \citeauthor{goodssphot} \citeyear{goodssphot}; UDS, \citeauthor{udsphot} \citeyear{udsphot}; COSMOS, \citeauthor{cosmosphot} \citeyear{cosmosphot}). In particular, an adapted form of Source Extractor has been used to more cleanly determine backgrounds and provide improved flux measurements. As a result, many source magnitudes have been revised to fainter values: the average source magnitude in the sample is fainter by $0.35$ mag. The faintest detected source in the revised catalog has a magnitude of $H = 28.3$.
...
Otherwise, we only incorporated photometry into our analysis after the collection of classifications was complete. In particular, we use $H$-band {\sc auto} magnitude, 80 per cent flux radius, and photometric redshifts in Section \ref{sec:biases} when discussing depth corrections and classification biases below. 
...
\begin{itemize}
\item \textbf{Classifier}. Those classifying galaxies within the Galaxy Zoo software\footnote{All classifications discussed here were collected via web software.} are essential to the success of the project. While it is true that the software is written so that a classifier could in principle be a machine, all classifiers referenced in this paper are human volunteers.
...
Second, it focuses the classifier on a single feature at a time, highlighting each feature. This resets the attention of the classifier with each new question and avoids the problems that may result when a person is presented with a large number of decision tasks at once, including a decrease in optimal decision-making \citep{references, for, overchoice} and a reduced ability to recognise the unexpected \citep{references_for,inattentional_blindness}. 
...
The values in Figure \ref{fig:classification_basic_info} are raw classification counts; while raw classification counts and vote fractions are certainly useful, we additionally apply a classifier weighting scheme to classifications to produce a cleaner set of vote fractions for each subject. The classifier weighting is described in further detail below.
...
%%%%% [FIGURE: User Consistencies] %%%%%
\begin{figure}
\includegraphics[scale=0.64]{consistencies_iterations2.eps}
\caption{
...
\subsubsection{Accounting for imaging of varying depths}\label{sec:depthcorr}
...
Figure \ref{fig:candels_compare}b compares these fractions for each galaxy in the same way as Figure \ref{fig:candels_compare}a, with darker shaded bins representing a higher number of galaxies within that bin, and with red squares indicating the median binned in both directions. The striations seen in the figure reflect the finite number of possible vote fractions within the CANDELS team votes; this structure was not seen in Figure \ref{fig:candels_compare}a due to the weighted combination of clumpy vote fractions. 
...
% want to compare to GZ2 / lackner & gunn sersic comparison, plus possibly GZ3 as well?
...
The Galaxy Zoo project has collected typically 40 or more independent visual classifications to date from colour images of 3 CANDELS fields: GOODS-South, COSMOS, and the UDS. Here we present the public release of these classifications, after applying an iterative consensus-based classifier weighting scheme that has been successfully applied to multiple previous Galaxy Zoo projects. We provide an analysis of changes in classifications with observed surface brightness and offer caveats and advice for usage of these morphological measurements for different science goals. Comparison of the Galaxy Zoo morphologies with existing visual morphologies of a subset of the sample are in excellent agreement across a wide range of morphological features. 
