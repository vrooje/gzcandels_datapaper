...
However, the Smooth galaxies at $1 \leq z \leq 3$ have a considerably more uniform distribution of bulge-to-total ratios than smooth galaxies at $z \sim 0$. A K-S test indicates a high level of significance ($7.6 \sigma$). We note that some of this difference is likely due to the fact that the studies performing bulge-disk decompositions of galaxies in each sample use slightly different methods to do so. For example, \citet{haeussler13} fit an exponential disk and a bulge with a variable S\'ersic index $n$ for each bulge-disk decomposition, whereas \citet{lackner12} force bulge-disk decomposition fits to have $n=1$ or $n=4$. Additionally, where \citet{lackner12} have determined that a single-component exponential or deVaucouleur fit is best for a particular galaxy, we take B/Tot$=0$ and $1$, respectively; this option is not available for the CANDELS fits, which likely explains some of the differences in smooth galaxies on the high B/Tot end. We choose to show in Figure \ref{fig:btot} the published best fits to each galaxy from among the multiple fits performed for each galaxy in each sample, despite these differences, and note that were we to only draw from the 2-component fits of \citet{lackner12} for the SDSS fits (which in many cases substantially disagrees with the best-fit model), the distributions would remain significantly different ($p = 0.026$).
, such that 23\% of galaxies in the Smooth sample have disk-dominated light profiles (B/Tot~$\leq 0.25$). 
