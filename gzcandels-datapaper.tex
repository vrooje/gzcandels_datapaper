...
{\notebsm
Resolution gets worse for a while in the redshifts we care about, but then it basically flattens out. At its worst it's X, which means that we have Y per pixel, or Z kpc resolution at worst given 2.5 pixels FWHM PSF for HST.
So we may not be resolving features smaller than that, but this depends somewhat on redshift and is a stronger function of z for z less than 1.

This resolution is slightly (better) than what Euclid \citep{refregier10} will get. Why are we mentioning Euclid specifically?
}
