...
In order to meaningfully compare disk fractions between samples of different morphologies and redshifts, we require that {\changed both featured and smooth} samples be matched in terms of how they sample the luminosity function of galaxies within their redshift ranges. Figures \ref{fig:dM}a and \ref{fig:dM}b show that the samples all have very different distributions of $\Delta M$. {\changed We therefore weight galaxies in each samples such that the weighted histograms of $\Delta M$ are statistically indstinguishable, as shown in figure \ref{fig:dm2}. This method has the advantage of considering all the galaxies in the sample (with appropriate weights) rather than relying on randomly selected sub-samples.}
